...
// These will be replaced by the Vite plugin at load time
const root = "__ROOT__";
const base = "__BASE__";
...
const getMaxZIndex = (target: HTMLElement, current: number): number => {
...
const clearOverlay = () => {
  ...
  if (current) delete (current as HTMLElement).dataset["clickToComponentTarget"];
